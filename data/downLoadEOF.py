	...
	def Search(self) -> list:
		...
		if n == 0:
			...
			for k in range(0, n, r):
				...
				for i in jsonInfo['value']:
					if i["Name"][2] != 'A':
						continue
					SearchResult.append({"Id": i["Id"], "Name": i["Name"]})
		# 检索数据
	...
	def Download1(self, DownloadInfo: list):
		...
		try:
			...
			if response.status_code == 200:
				...
				with open(f'{tempPath}', 'wb') as f:
					for data in tqdm(iterable=response.iter_content(1024 * 1024), total=int(data_size),
									 desc=f'{tempPath}', unit='MB'):
						f.write(data)
				shutil.move(f'{tempPath}', f'{savePath}')
...
if __name__ == '__main__':
	# todo 设置参数
	IPPort = "127.0.0.1:7890"  # todo 设置代理

	Folder = r"../dataset/dataEOF"  # todo 保存路径
	userName = "frevoid28208@gmail.com"  # todo 用户名
	password = "Li@@20021114"  # todo 密码
	...
	# 读取urlString
	with open("../dataset/SearchURL.txt", mode="r") as f:
	...
	# 下载数据
	SL.MultiDownload(saveFolder=Folder,poolNum=6)  # 多进程
